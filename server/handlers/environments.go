...
func GetEnvironments(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := database.DB.Query(`
		SELECT id, project_id, name, api_key, settings, is_active, created_at 
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.ProjectID, &e.Name, &e.APIKey, &e.Settings, &e.IsActive, &e.CreatedAt); err != nil {
			continue
		}
		envs = append(envs, e)
	}
...
func CreateEnvironment(w http.ResponseWriter, r *http.Request) {
	...
	err = database.DB.QueryRow(`
		...
		VALUES ($1, $2) 
		RETURNING id, project_id, name, api_key, settings, is_active, created_at
	`, projectID, input.Name).Scan(&e.ID, &e.ProjectID, &e.Name, &e.APIKey, &e.Settings, &e.IsActive, &e.CreatedAt)
...
func UpdateEnvironment(w http.ResponseWriter, r *http.Request) {
	...
	query += fmt.Sprintf(" WHERE id = $%d AND project_id = $%d RETURNING id, project_id, name, api_key, settings, is_active, created_at", argIdx, argIdx+1)
	args = append(args, envID, projectID)

	var e models.Environment
	err = database.DB.QueryRow(query, args...).Scan(&e.ID, &e.ProjectID, &e.Name, &e.APIKey, &e.Settings, &e.IsActive, &e.CreatedAt)

	if err != nil {
		http.Error(w, "Environment not found or update failed", http.StatusNotFound)
		return
	}
